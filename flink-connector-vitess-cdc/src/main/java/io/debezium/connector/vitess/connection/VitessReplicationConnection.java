...
public class VitessReplicationConnection implements ReplicationConnection {
     ...
     * @throws StatusRuntimeException if the connection is not valid, or SQL statement can not be
    ...
    public void execute(String sqlStatement) {
        ...
        Vtgate.ExecuteRequest request =
                Vtgate.ExecuteRequest.newBuilder()
    ...
    public void startStreaming(
        ...
        LOGGER.info("Event failure handling mode:" + config.getEventProcessingFailureHandlingMode().getValue());

        ManagedChannel channel = newChannel(config.getVtgateHost(), config.getVtgatePort());
        managedChannel.compareAndSet(null, channel);

        VitessGrpc.VitessStub stub = newStub(channel);
        StreamObserver<Vtgate.VStreamResponse> responseObserver =
                new StreamObserver<Vtgate.VStreamResponse>() {
                    ...
                    public void onNext(Vtgate.VStreamResponse response) {
                        ...
                        LOGGER.debug(
                                "Received {} vEvents in the VStreamResponse:",
                    ...
                    public void onError(Throwable t) {
                        ...
                        if (isVitessEofException(t)) {
                            ...
                            // Mitigate vgtid expired with EOF exception in case SKIP is enabled
                                    ...
                                    && lastErrorVgtid.equals(lastProcessedVgtid != null ? lastProcessedVgtid : vgtid)
                                    && config
                                    .getEventProcessingFailureHandlingMode()
                                    .equals(CommonConnectorConfig.EventProcessingFailureHandlingMode.SKIP)) {
                            ...
                            lastErrorVgtid = lastProcessedVgtid != null ? lastProcessedVgtid : vgtid;
                        } else {
                            LOGGER.error(
                    ...
                    // Even in case of resharding, there is only one vgtid event that contains
                    // multiple shard
                    // gtids.
                    private Vgtid getVgtid(Vtgate.VStreamResponse response) {
                        ...
                        if (vgtids.size() == 0) {
                            // The VStreamResponse that contains an VERSION vEvent does not have
                            // VGTID.
                            // We do not update lastReceivedVgtid in this case.
                            // It can also be null if the 1st grpc response does not have vgtid upon
                            // restart
                            LOGGER.trace(
                                    "No vgtid found in response {}...",
                                    response.toString()
                                            .substring(
                                                    0,
        ...
        if (config.getVtgateUsername() != null && config.getVtgatePassword() != null) {
            ...
            stub =
                    stub.withCallCredentials(
                            new StaticAuthCredentials(
                                    config.getVtgateUsername(), config.getVtgatePassword()));
        }
        return stub;
    }
    ...
    private ManagedChannel newChannel(String vtgateHost, int vtgatePort) {
        ManagedChannel channel =
                ManagedChannelBuilder.forAddress(vtgateHost, vtgatePort)
                        .usePlaintext()
    ...
    public static Vgtid defaultVgtid(VitessConnectorConfig config) {
        if (config.getShard() == null || config.getShard().isEmpty()) {
            ...
            LOGGER.info(
                    "Default VGTID is set to the current gtid of all shards from keyspace: {}",
                    config.getKeyspace());
            return Vgtid.of(
            ...
            try (VtctldConnection vtctldConnection =
                    VtctldConnection.of(
    ...
    public String connectionString() {
        return String.format(
                "vtgate gRPC connection %s:%s", config.getVtgateHost(), config.getVtgatePort());
    }
    ...
    private static Topodata.TabletType toTopodataTabletType(
            VtctldConnection.TabletType tabletType) {
        switch (tabletType) {
            case MASTER:
